    ...
    <section id="skills" className="bg-bg px-[5vw] py-24">
      ...
      <div className="flex flex-col md:flex-row gap-10 md:gap-14">
            ...
            {skillGroups.map(g => (
              <li key={g.id}>
                <button
                  ...
                  className={`font-pixel text-[0.37rem] tracking-widest px-3 py-[0.75rem] w-full text-left whitespace-nowrap transition-all duration-300
        ...
        <div className="flex-1 flex flex-col gap-10">
          {skillGroups.map(g => (
            <div key={g.id}
              className={`transition-opacity duration-300 ${active === g.id ? 'opacity-100' : 'opacity-50'}`}>
              <div className="flex items-center gap-3 mb-4 pb-3 border-b border-[var(--border)]">
                <span className="font-display text-[2rem] leading-none" style={{color:'rgba(192,18,43,0.22)'}}>
                  {g.num}
                </span>
