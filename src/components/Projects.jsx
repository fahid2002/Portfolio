...
function FigmaCard({ design, i }) {
  return (
    <motion.div
      ...
      <div className="font-pixel text-[0.33rem] leading-[2.2] tracking-widest" style={{color:'rgba(200,240,74,0.5)'}}>
        {design.desc}<br/>
    ...
    <section id="projects" className="bg-bg2 px-[5vw] py-24">
      ...
      {projectCategories.map(cat => (
        <div key={cat.id} className="mb-16">
          {/* Category header */}
          <div className="flex items-center gap-3 mb-7 pb-4 border-b border-[var(--border)]" data-aos="fade-up">
            <span className="text-[1.4rem]">{cat.icon}</span>
            <span className="font-display text-[2rem] text-cream tracking-[0.04em]">{cat.title}</span>
            <span className={`font-pixel text-[0.36rem] border px-2 py-1 ${
              cat.isFigma
